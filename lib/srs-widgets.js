...
var getNextGame = function(team_id) {
	...
	jQuery.ajax({
	    ...
	    success: function(data){
          ...
          var logoRow = hostLogo + ' - ' + guestLogo + '<br/>';
          var teamRow = "<a href='http://swissrugbystats.ch/games/" + data.id + "' target='_blank'>" + data.host.team.name + " vs " + data.guest.team.name + "</a><br/>"
          var d = parseDate(data.date);
          var kickOffRow = getDayOfWeek(d) +", "+getDateString(d) + ", " +getKickoffTimeString(d);
          jQuery(selector).html(hostLogo + " - " +guestLogo + "<br />" + data.host.team.name + " vs " + data.guest.team.name + "<br/>" +  dStr);
	    }
	});
}
...
var getLastGame = function(team_id) {
    ...
    jQuery.ajax({
        ...
        success: function(data){
            ...
            jQuery(selector).html(data.host.team.name + " vs " + data.guest.team.name + "<br/>" +  dStr);
        }
    });
}
